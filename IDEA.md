...
- Below it: a large rounded glass input (radius 28, height 56, frosted), with:
  ...
  - right-aligned **press-and-hold mic icon** wired to `speech_to_text`: user holds to talk, releases to stop. On press the text field is cleared so the new dictation starts fresh (no visual flicker as the first recognition result lands). Partial recognition results stream into the text field in real time. On release, the next final result **auto-submits** — the search fires without an Enter tap. While listening the input border + mic icon glow brand-primary, the mic scales up 1.18×, and the keyboard is dismissed. Permissions are pre-warmed during the splash hold (see §3.1); if permission is denied or the engine reports a permanent error the mic dims to `surface.outline` and stops accepting input.
...
│   │   │   ├── ranking_cache.dart          # hive_ce-backed, box `ranking_cache_v3`, LRU 50
...
- **Resolution — Wikipedia enrichment:** after OpenAI returns titles, `WikipediaImageLookup` runs a two-step REST flow per item — (1) `/w/rest.php/v1/search/page?q={title}&limit=1` resolves the actual Wikipedia page title (handles disambiguation, e.g. "Hereditary" → "Hereditary (film)"), (2) `/api/rest_v1/page/summary/{resolved}` returns both the thumbnail URL **and** infobox coordinates when present. For `PlaceItem`, the lat/lng from Wikipedia replace the model's hallucinated coordinates (GPT-4o-mini guesses plausible-looking numbers that don't actually point at the place). The single-call MediaWiki `prop=pageimages` API was tried first and returned empty thumbnails for most pages, even when a representative image clearly exists on the page. Free, no auth, CORS-friendly.
- **Rate-limit handling:** Wikipedia's REST API rate-limits per-IP aggressively. 10 fully-parallel items × 2 calls each (20 hits in <100ms) trips 429s instantly. Mitigated by (a) sending a descriptive `User-Agent` (`CountdownApp/1.0 (Flutter; Dart/Dio)`), (b) processing items **fully sequentially** — one at a time, no concurrency. Parallel batches of 2 still failed because retries stacked: multiple in-flight 429'd requests all retried at the same backoff moment, re-hammering the limit. Sequential adds latency (~3s for 10 items vs ~1s parallel) but lifts the hit rate dramatically. `dio_smart_retry` still installed for transient blips.
...
- ✅ **Splash screen** (`lib/features/splash/splash_screen.dart`) — the app's `home:`. Centered Fraunces "Countdown" wordmark with a brand.primary pulsing dot after the "n" (1.2s loop). Auto-replaces itself with the Search screen after 1s via a fade-through `PageRouteBuilder` (no back navigation). The native iOS LaunchScreen storyboard renders the logo image during boot, then this Flutter splash takes over once the engine is live.
- ✅ **Search screen** (`lib/features/search/search_screen.dart`) — the destination after Splash. Headline + glass input with rotating placeholder (cycles 6 example queries every 3s with fade-slide animation) + a clear-text × button (only when input is non-empty) + tappable example chips (auto-submit on tap) + footer credit. Submits via keyboard `search` action or chip tap, pushes `RankingScreen` via `MaterialPageRoute`. Mic affordance present but disabled (voice-input is stretch).
