...
- **Image enrichment:** **`WikipediaImageLookup`** — two-step REST per item (search → summary), processed **sequentially** (~300ms each, ~3s for 10 items). Returns both the thumbnail URL and (for places) the page's real `coordinates.{lat,lon}`, replacing GPT's hallucinated lat/lng on `PlaceItem`. Lives between OpenAI and the drip in `RankingRepository`. Parallel was attempted and reliably hit Wikipedia's 429 limiter — retries stacked.
...
- **Voice input:** `speech_to_text` ^7.4 — press-and-hold mic on Search; partial results stream into the text field. iOS Speech framework. Mic + speech permissions are in `Info.plist`. Engine is **pre-warmed during the splash hold** (5s) so the iOS permission popups land while the splash is up, not when the user is trying to record. `_QueryInputState` re-runs `initialize()` defensively but sees granted state and skips the popups.
